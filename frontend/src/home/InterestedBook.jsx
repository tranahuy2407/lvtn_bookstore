...
const InterestedBook = () => {
  ...
  const fetchBooks = async (userId) => {
    try {
      const currentBookId = ''; // Provide the current book ID if available
      const response = await fetch(`http://localhost:5000/api/books/interested?userId=${userId}&currentBookId=${currentBookId}`);
      if (!response.ok) {
        throw new Error('Failed to fetch data');
      }
      const data = await response.json();
      setBooks(data.books.slice(0, 8)); // Adjust based on API response structure
    } catch (error) {
    ...
    <div className='container mx-auto my-16 px-4 lg:px-24 bg-white'>
      <div className='py-16 sm:py-24'>
        <div className='flex items-center justify-between'>
          <h2 className='text-2xl font-bold tracking-tight text-gray-900'>Có lẽ bạn quan tâm</h2>
